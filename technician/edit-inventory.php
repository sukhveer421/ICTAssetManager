															...
															<div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<select id="Custodian" name="Custodian" class="select2" data-placeholder="Click to Choose...">
																		...
																		<?php
																			$stmt = $db->query("SELECT Id, Role, Name FROM profilemaster WHERE Status = 'Y' AND Role <> 4");
																			while($row = $stmt->fetch())
																			{
																				?>
																				<option value="<?php echo $row['Name'];?>" <?php if( $row_fechAsset['Custodian']==$row['Name']){ echo "selected = 'true'";} ?>> <?php echo $row['Name'];?></option>
																				<?php
																			}
																		 ?>
																	</select>
